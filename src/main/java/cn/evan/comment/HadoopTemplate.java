...
public class HadoopTemplate {
    ...
    public HadoopTemplate(Configuration conf, String defaultHdfsUri) {
        this.conf = conf;
            ...
            try {
                ...
                if (statuses != null) {
                    if (statuses.length>1){
                        log.info("文件发生变化，需要处理");
                        for (FileStatus status : statuses) {
                            if (status.getPath().getName().equalsIgnoreCase("products.csv")){
                                continue;
                            }
                            System.out.println(status.getPath().getName());
                            try {
                                String open = openWithString(status.getPath());
                                delete(status.getPath());
                            }catch (Exception e){
                                e.printStackTrace();
                            }
                        }
     ...
     * @return FSDataInputStream
     */
    public String openWithString(Path path) {
